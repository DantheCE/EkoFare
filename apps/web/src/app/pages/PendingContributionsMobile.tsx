 ...
 * No library needed for common cases: seconds, minutes, hours, days.
 */
...
/** Sum all leg fares (index 0 = origin, always 0) */
...
// ─────────────────────────────────────────────────────────────────────────────
// ConfirmationsBar — filled dots for verification progress
// ─────────────────────────────────────────────────────────────────────────────

function ConfirmationsBar({ count, total = 3 }: { count: number; total?: number }) {
  return (
    <div
...
// ─────────────────────────────────────────────────────────────────────────────
// Stop Breakdown — read-only leg-by-leg timeline (no tap-to-select)
// ─────────────────────────────────────────────────────────────────────────────

function StopBreakdown({ contribution }: { contribution: Contribution }) {
  return (
    <div
      style={{
...
// ─────────────────────────────────────────────────────────────────────────────
// PendingCard — a single contribution card with vote controls
// ─────────────────────────────────────────────────────────────────────────────
...
function PendingCard({ contribution, deviceId, onAnimateOut }: PendingCardProps) {
  ...
  const stopCount = contribution.stops_data.length - 1; // exclude origin
  ...
  async function handleVote(type: "confirm" | "dispute") {
    ...
    try {
      if (type === "confirm") {
        ...
        // Not yet rejected (needs more disputes)
        void res;
      }
    } finally {
      setVoting(false);
...
export default function PendingContributionsMobile() {
  const { data: contributions = [], isLoading } = useContributions("pending");
